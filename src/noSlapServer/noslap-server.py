...
slap_file = os.path.join(path, "no-slaps.json")
...
def home():
    ...
    return render_template('app/home.html', noslaps=slaps,
...
def edit_noslap(slapid):
    if (request.method == 'POST') and ("day" in request.form):
        ...
        if slapid < len(noslaps["NOSLAPS"]):  # in this case, a no slap is edited
            ...
            endtime = "08:00"
            def_volume = "100"
        return render_template('app/edit-noslap.html', select_day=select_day,
...
def my_no_slaps():
    ...
    if noslaps["NOSLAPS"] == []:
        ...
        noslaps_string = ""
    return render_template('app/no-slaps.html', noslaps=noslaps["NOSLAPS"],
...
def edit_filaments():
    if request.method == 'POST':
        ...
        settings = open(settings_file).read()
        return render_template('app/settings.html', settings=settings)
